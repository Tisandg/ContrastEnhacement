...
void run_cpu_color_test(unsigned char *segment_in_c_r, unsigned char *segment_in_c_g, unsigned char *segment_in_c_b, int segment_size_c, int width, int length);
...
int main(int argc, char** argv){
    ...
    int img_size_c, segment_size_c, remainder_c, width_color, height_color;
    ...
    if (rank == 0) {
        ...
        width      = img_ibuf_g.w;
        height     = img_ibuf_g.h;
        width_color = img_ibuf_c.w;
        height_color = img_ibuf_c.h;
        img_size_g = img_ibuf_g.w * img_ibuf_g.h;
        img_size_c = img_ibuf_c.w * img_ibuf_c.h;
        printf("width of input image= %d\n",width_color);
        printf("height of input image= %d\n",height_color);
    }
    ...
    MPI_Bcast(&width_color, 1, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Bcast(&height_color, 1, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    int rows_per_process = height_color / size;
    int extra_rows = height_color % size;

    // Each process calculates its own segment dimensions
    int segment_rows = (rank < size - 1) ? rows_per_process : (rows_per_process + extra_rows);
    segment_size_c = segment_rows * width_color; // Number of pixels in the segment

    // Allocate memory for each segment
    // segemnts of gray image
    unsigned char *segment_in_g = (unsigned char *)malloc(segment_size_g * sizeof(unsigned char));
    // segments of color image
    ...
    // COLOR
    run_cpu_color_test(segment_in_c_r, segment_in_c_g, segment_in_c_b, segment_size_c, width_color, height_color);
    free(segment_in_c_r);
    free(segment_in_c_g);
    free(segment_in_c_b);
...
void run_cpu_color_test(unsigned char *segment_in_c_r, unsigned char *segment_in_c_g, unsigned char *segment_in_c_b, int segment_size_c, int width, int length)
{
    ...
    img_obuf_hsl = contrast_enhancement_c_hsl(segment_in_c_r, segment_in_c_g, segment_in_c_b, segment_size_c, width, length);

    if (rank==0){
        write_ppm(img_obuf_hsl, "out_hsl.ppm");
    }

    printf("width of output image= %d\n",img_obuf_hsl.w);
    printf("height of output image= %d\n",img_obuf_hsl.h);
    // img_obuf_yuv = contrast_enhancement_c_yuv(segment_in_c_r, segment_in_c_g, segment_in_c_b, segment_size_c, width, length);
